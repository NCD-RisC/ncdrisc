...
clean_data <- function(data, variable, omit_values = c() ) {
  ...
  if (length(clean_list) > 0) {
    message(paste0('Pre-cleaning variable "', var_name, '":'))
...
clean_data_index <- function(data, variable) {
  clean_list <- switch(variable,
                       ...
                       is_fasting =
                         clean_categorical(data[[variable]], variable, c(0,1)),
                       fasting_time =
                         clean_continuous(data[[variable]], variable, 6, 24),
...
print_message <- function(var_name, clean_list, var, id_study) {
  print(paste("Percentage Cleaned (No. of cleaned/No. of non-NAs):", var_name, "(%)"))
  if (length(clean_list)==0) {
